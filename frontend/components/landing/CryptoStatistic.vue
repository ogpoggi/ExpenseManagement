<template>
  <div class="w-full lg:w-1/3 mt-6 lg:mt-0 overflow-hidden space-y-6 dark:bg-gray-800" v-bind="$attrs">
      ...
      <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
        <div class="px-2 sm:px-6 py-2 align-middle inline-block min-w-full overflow-hidden">
          <table class="min-w-full">
            <thead>
              <tr>
                <th class="text-left text-sm font-medium text-gray-500 dark:text-[#468ef9]">Name</th>
                <th class="text-left text-sm font-medium text-gray-500 dark:text-[#468ef9]">Price</th>
                <th class="hidden sm:block text-left text-sm font-medium text-gray-500 dark:text-[#468ef9]">Chart</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="data in datasets" :key="data.id" class="border-b border-gray-200">
                <td class="py-4 whitespace-nowrap">
                  <div class="flex items-center space-x-2">
                    ...
                    <span class="dark:text-white">{{ data.name }}</span>
                  </div>
                </td>
                <td class="py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <PlusThickIcon v-if="data.increase" :size="14" class="text-green-500" />
                    <MinusThickIcon v-else :size="14" class="text-red-500" />
                    <span class="dark:text-white">${{ data.price }}</span>
                  </div>
                </td>
